...
public class ClassArray extends ValueType {
	...
	public ClassArray(ClassType type) {
		this.type = type;
		this.values = null;
	...
	private ClassArray(ClassType type, int dimension[]) throws BugTrap {
		this.type = type;
	...
	private Object[] allocate(int[] indices, int i) throws BugTrap {
		if (i==indices.length-1) {
			ClassType[] level0 = new ClassType[indices[i]];
	...
	public static java.lang.String indicesToString(int indices[]) {
		java.lang.String s = "";
		for (int i=0; i<indices.length; i++) s += ClassArray.SquareOpen + indices[i] + ClassArray.SquareClose;
		return s;
	}
	...
	public void setElement(ClassType newValue, int[] indices) throws BugTrap {
		if (newValue.getType()!=type.getType()) throw new BugTrap(this, "Array assignment type mismatch.");
		...
		((Object[]) ptr)[indices[dim-1]] = newValue;
	}

	public ClassType getElement(int[] indices) {
		int dim = indices.length;
		Object ptr = values;
		for (int index=0; index<dim-1; index++) {
			ptr = ((Object[]) ptr)[indices[index]];
		}
		return (ClassType) ((Object[]) ptr)[indices[dim-1]];
	}
	
	
	
	
	public static ClassArray parse(ClassType type, java.lang.String nameAndDimension, ArrayList<Expression> initialValues, Context context) throws BugTrap {
		LineTokenizer tokenizer = new LineTokenizer(nameAndDimension);
		if (!tokenizer.isArrayName()) throw new BugTrap(context, "Array bad syntax.");
		...
		ClassArray array = new ClassArray(type, dimension);
		...
		if (initialValues!=null) {
			for (int i=0; i<initialValues.size(); i++) {
				...
				ClassType newValue = (ClassType) initialValues.get(i).evaluate(context);
	...
	public compiler.data.Integer toInteger() throws BugTrap {
	...
	public compiler.data.Character toCharacter() throws BugTrap {
		throw new BugTrap(this, "not convertible to Character");
	}
	public ClassArray copy() throws BugTrap {
		ClassArray array = new ClassArray(type.classBlock.getType());
