...
public class IntegerToString extends LibFunction {

	// String integerToString(Integer value)
	public static final String name = "integerToString";
	private static final String Param = "value";
	public static final String parameters = compiler.data.Integer.StartKeyword + Token.space + Param;
	public static final String returnType = compiler.data.String.StartKeyword;

	
	public IntegerToString() throws BugTrap {
	...
	public int execute(Context context) throws BugTrap {
		...
		if (value instanceof compiler.data.Integer) {
			int x = ((compiler.data.Integer) value).getValue();
			setReturnValue( new compiler.data.String(""+x) , context);
		}
		else {
